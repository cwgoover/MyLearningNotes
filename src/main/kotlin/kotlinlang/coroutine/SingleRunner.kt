...
class SingleRunner {
    ...
    suspend fun <T> afterPrevious(block: suspend () -> T): T {
         ...
         *
         * It uses a Mutex, which is a single ticket (or lock), that a coroutine must get in order to
         * enter the block. If another coroutine tried while one was running, it would suspend itself
         * until all pending coroutines were done with the Mutex.
         */
    }
}
...
class ControlledRunner<T> {
    ...
    suspend fun cancelPreviousThenRun(block: suspend() -> T): T {
         ...
         * https://medium.com/androiddevelopers/coroutines-on-android-part-iii-real-work-2ba8a2ec2f45
         *
         * In a nutshell it always keeps track of the currently active sort in the member variable [activeTask].
         * Whenever a sort starts, it will immediately [cancelAndJoin] on whatever is currently in [activeTask].
         * This has the effect of cancelling any in progress sorts before starting a new one.
         */
